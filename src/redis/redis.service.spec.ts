...
describe('RedisService', () => {
  ...
  beforeEach(async () => {
    ...
    (service as any).keyPrefix = 'coaches';
  });
  ...
  describe('get', () => {
    it('should get value from Redis', async () => {
      ...
      expect(mockRedisClient.get).toHaveBeenCalledWith('coaches:test-key');
  ...
  describe('set', () => {
    it('should set value without TTL', async () => {
      ...
      expect(mockRedisClient.set).toHaveBeenCalledWith('coaches:test-key', 'test-value');
    });

    it('should set value with TTL', async () => {
      await service.set('test-key', 'test-value', 3600);

      expect(mockRedisClient.setEx).toHaveBeenCalledWith('coaches:test-key', 3600, 'test-value');
    });
  });
  ...
  describe('del', () => {
    it('should delete key from Redis', async () => {
      ...
      expect(mockRedisClient.del).toHaveBeenCalledWith('coaches:test-key');
  ...
  describe('expire', () => {
    it('should set expiration on key', async () => {
      ...
      expect(mockRedisClient.expire).toHaveBeenCalledWith('coaches:test-key', 3600);
  ...
  describe('exists', () => {
    it('should return true when key exists', async () => {
      ...
      expect(mockRedisClient.exists).toHaveBeenCalledWith('coaches:test-key');
      expect(result).toBe(true);
    });
  ...
  describe('keys', () => {
    it('should get keys matching pattern', async () => {
      ...
      expect(mockRedisClient.keys).toHaveBeenCalledWith('coaches:test:*');
  ...
  describe('incr', () => {
    it('should increment value', async () => {
      ...
      expect(mockRedisClient.incr).toHaveBeenCalledWith('coaches:counter');
      expect(result).toBe(1);
    });
  });
  ...
  describe('decr', () => {
    it('should decrement value', async () => {
      ...
      expect(mockRedisClient.decr).toHaveBeenCalledWith('coaches:counter');
  ...
  describe('getPrefixedKey', () => {
    it('should add prefix to key', async () => {
      ...
      expect(mockRedisClient.get).toHaveBeenCalledWith('coaches:my-key');
    });
  });
});
