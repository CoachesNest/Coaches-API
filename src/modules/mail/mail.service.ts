...
export class MailService {
  ...
  async sendLoginEmail(email: string): Promise<void> {
    ...
    //   to: email,
    //   subject: 'Successful Login to Coaches',
  ...
  async sendWelcomeEmail(user: MailUser): Promise<void> {
    try {
      ...
      const templateVars = {
        ...
        appUrl: process.env.APP_URL || 'https://coaches.com',
  ...
  async sendLoginEmail(user: MailUser, metadata?: LoginMetadata): Promise<void> {
    try {
      ...
      const templateVars = {
        ...
        email: this.maskEmail(email),
        appName: this.configService.mailAppName,
        resetUrl: process.env.RESET_PASSWORD_URL || 'https://coaches.com/reset-password',
