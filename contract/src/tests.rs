...
fn test_initialize_and_get_treasury() {
    ...
    CoachesContract::initialize(env.clone(), admin.clone(), initial_treasury.clone(), native_token.clone());

    // Verify we can get the treasury
    let stored_treasury = CoachesContract::get_treasury(env.clone());
    assert_eq!(stored_treasury, initial_treasury);

    // Verify we can get the admin
    let stored_admin = CoachesContract::get_admin(env.clone());
    assert_eq!(stored_admin, admin);

    // Verify we can get the native token
    let stored_native_token = CoachesContract::get_native_token(env.clone());
...
fn test_cannot_initialize_twice() {
    ...
    CoachesContract::initialize(env.clone(), admin, initial_treasury, native_token);
}
...
fn test_set_treasury() {
    ...
    // Initialize
    CoachesContract::initialize(env.clone(), admin.clone(), initial_treasury.clone(), native_token);

    // Update treasury
    CoachesContract::set_treasury(env.clone(), new_treasury.clone());

    // Verify the new treasury is set
    let updated_treasury = CoachesContract::get_treasury(env.clone());
...
fn test_non_admin_cannot_set_treasury() {
    ...
    CoachesContract::set_treasury(env.clone(), new_treasury);
}
...
fn test_lock_funds() {
    ...
    // Initialize the contract
    CoachesContract::initialize(env.clone(), admin, treasury, native_token.clone());

    // Call lock_funds as buyer
    env.set_invoker(buyer.clone());
    CoachesContract::lock_funds(env.clone(), session_id.clone(), seller.clone(), amount);

    // Verify the session exists and has correct values
    let session = CoachesContract::get_session(env.clone(), session_id.clone());
...
fn test_lock_funds_rejects_zero_amount() {
    ...
    CoachesContract::initialize(env.clone(), admin, treasury, native_token);
    env.set_invoker(buyer);
    // Try to lock 0 amount - should panic
    CoachesContract::lock_funds(env.clone(), session_id, seller, 0i128);
}
...
fn test_lock_funds_rejects_duplicate_session_id() {
    ...
    CoachesContract::lock_funds(env.clone(), session_id.clone(), seller1, 1000i128);
    // Second lock with same session ID should panic
    CoachesContract::lock_funds(env.clone(), session_id, seller2, 500i128);
}
...
// (soroban_sdk / CoachesContract) and this file is not currently wired into
...
// The tests below target the current Anchor `coaches` program directly.
