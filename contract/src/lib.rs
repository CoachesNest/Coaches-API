...
/// Error codes for CoachesEscrow (#526, financial validation errors)
#[contracterror]
...
impl CoachesEscrow {
    ...
    pub fn set_platform_fee(env: Env, new_fee_bps: u32) {
        ...
        env.storage()
            .persistent()
            .set(&CoachesKey::PlatformFee, &new_fee_bps);
        env.events()
    ...
    pub fn set_treasury(env: Env, new_treasury: Address) {
        ...
        env.storage()
            .persistent()
            .set(&CoachesKey::Treasury, &new_treasury);
        env.events()
