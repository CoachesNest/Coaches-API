...
// ============================================================================
// Coaches Escrow Contract — issues #521 #522 #523 #525 #526 #527
// ============================================================================
...
/// Error codes for CoachesEscrow (#526)
#[contracttype]
#[derive(Clone, Copy, PartialEq, Debug)]
#[repr(u32)]
...
#[contract]
pub struct CoachesEscrow;

impl CoachesEscrow {
    fn get_session_internal(env: &Env, id: &Bytes32) -> SessionData {
        env.storage()
            .persistent()
            .get(&CoachesKey::Session(id.clone()))
    ...
    fn save_session_internal(env: &Env, id: &Bytes32, session: &SessionData) {
        env.storage()
            .persistent()
            .set(&CoachesKey::Session(id.clone()), session);
    }

    fn get_admin(env: &Env) -> Address {
        env.storage()
            .persistent()
            .get(&CoachesKey::Admin)
...
impl CoachesEscrow {
    pub fn initialize(env: Env, admin: Address) {
        if env.storage().persistent().has(&CoachesKey::Admin) {
            panic!("already initialized");
        }
        env.storage().persistent().set(&CoachesKey::Admin, &admin);
        env.storage().persistent().set(&CoachesKey::PlatformFee, &0_u32);
    }

    pub fn set_treasury(env: Env, new_treasury: Address) {
        Self::get_admin(&env).require_auth();
        env.storage().persistent().set(&CoachesKey::Treasury, &new_treasury);
    }

    pub fn get_treasury(env: Env) -> Address {
        env.storage().persistent().get(&CoachesKey::Treasury).expect("treasury not set")
    }
    ...
    pub fn set_platform_fee(env: Env, new_fee_bps: u32) {
        ...
        let old_fee_bps = env.storage().persistent().get(&CoachesKey::PlatformFee).unwrap_or(0_u32);
        env.storage().persistent().set(&CoachesKey::PlatformFee, &new_fee_bps);
        env.events().publish(
            (Symbol::new(&env, "PlatformFeeUpdated"),),
            (old_fee_bps, new_fee_bps, admin),
        );
    ...
    pub fn get_platform_fee(env: Env) -> u32 {
        env.storage().persistent().get(&CoachesKey::PlatformFee).unwrap_or(0_u32)
    }
    ...
    pub fn lock_funds(
        ...
        if env
            .storage()
            .persistent()
            .has(&CoachesKey::Session(session_id.clone()))
        {
    ...
    pub fn set_dispute_window(env: Env, window_ledgers: u32) {
        ...
        env.storage()
            .persistent()
            .set(&CoachesKey::DisputeWindow, &window_ledgers);
        env.events().publish(
