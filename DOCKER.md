# Docker Setup Guide for Coaches Server

This guide explains how to use Docker for development and production deployment of the Coaches backend API.
...
docker exec -it coaches-backend-dev sh

# Run tests inside container
docker exec coaches-backend-dev npm test

# Run migrations
docker exec coaches-backend-dev npm run migration:run
```
...
docker inspect --format='{{.State.Health.Status}}' coaches-backend
docker inspect --format='{{.State.Health.Status}}' coaches-postgres
docker inspect --format='{{.State.Health.Status}}' coaches-redis
```
...
docker logs coaches-postgres

# Access PostgreSQL shell
docker exec -it coaches-postgres psql -U postgres -d coaches_db
```
...
docker exec coaches-redis redis-cli -a redis_password ping
```
