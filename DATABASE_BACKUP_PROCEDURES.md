...
This document outlines the manual and automated backup and restore procedures for the Coaches PostgreSQL database. The backup system includes:
...
PGPASSWORD="your-db-password" createdb -h localhost -p 5432 -U postgres coaches_test
...
# Verify
PGPASSWORD="your-db-password" psql -h localhost -p 5432 -U postgres -d coaches_test -c "\dt"
```
...
```bash
PGPASSWORD="your-db-password" createdb -h localhost -p 5432 -U postgres coaches_verify_test

PGPASSWORD="your-db-password" pg_restore \
  -h localhost \
  -p 5432 \
  -U postgres \
  -d coaches_verify_test \
  --no-owner \
  --no-privileges \
  ./backups/backup_20240429_103000.sql.gz
```
...
5. **Clean up**:
```bash
PGPASSWORD="your-db-password" dropdb -h localhost -p 5432 -U postgres coaches_verify_test
```
...
```bash
PGPASSWORD="your-db-password" psql -h localhost -p 5432 -U postgres -d coaches_db -c "SELECT 1"
```
