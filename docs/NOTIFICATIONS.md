...
The email notifications system provides a flexible, provider-agnostic way to send emails for key events in the Coaches platform. The system is built with a pluggable adapter pattern, allowing you to switch between different email providers (SendGrid, Resend, AWS SES, etc.) without changing the application code.
...
© 2026 Coaches. All rights reserved.
```
...
APP_URL=https://staging.coaches.com
SENDGRID_FROM_EMAIL=noreply@staging.coaches.com
```
