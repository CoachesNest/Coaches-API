...
// ============================================================================
// Coaches Escrow Contract — original issues + new issues integrated
// ============================================================================
...
impl CoachesEscrow {
    ...
    pub fn initialize(env: Env, admin: Address, treasury: Address) {
        ...
        only_once!(&env, CoachesKey::Initialized);

        // Persist admin role
        env.storage()
            .persistent()
            .set(&CoachesKey::Admin, &admin);

        // Persist treasury address
        env.storage()
            .persistent()
            .set(&CoachesKey::Treasury, &treasury);
    ...
    pub fn upgrade(env: Env, new_wasm_hash: Bytes32) -> Result<(), EscrowError> {
        ...
        let old_wasm_hash = env
            .storage()
            .persistent()
            .get::<_, Bytes32>(&CoachesKey::WasmHash)
            .unwrap_or_else(|| zero_hash.clone());

        env.storage()
            .persistent()
            .set(&CoachesKey::WasmHash, &new_wasm_hash);
