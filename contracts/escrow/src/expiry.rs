...
use crate::{reentrancy, Bytes32, SessionData, CoachesEscrow, CoachesKey, Status};
...
/// `CoachesEscrow` writes to (same `CoachesKey::Session` keys), so no
/// cross-contract call is needed.
pub fn cancel_expired_session(env: Env, session_id: Bytes32, token_id: Address) {
    // Retrieve session from the main CoachesEscrow persistent storage
    let mut session: SessionData = env
        .storage()
        .persistent()
        .get(&CoachesKey::Session(session_id.clone()))
    ...
    env.storage()
        .persistent()
        .set(&CoachesKey::Session(session_id.clone()), &session);
