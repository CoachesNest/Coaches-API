...
pm2 stop all
# OR
sudo systemctl stop coaches-server
```
...
PGPASSWORD="your-db-password" dropdb -h localhost -p 5432 -U postgres coaches_db

# 2. Create fresh database
PGPASSWORD="your-db-password" createdb -h localhost -p 5432 -U postgres coaches_db
```
...
# 4. Verify restore
PGPASSWORD="your-db-password" psql -h localhost -p 5432 -U postgres -d coaches_db -c "\dt"
PGPASSWORD="your-db-password" psql -h localhost -p 5432 -U postgres -d coaches_db -c "SELECT count(*) FROM users;"
```
...
pm2 start all
# OR
sudo systemctl start coaches-server

# Check application logs
pm2 logs
# OR
sudo journalctl -fu coaches-server
```
...
PGPASSWORD="your-db-password" psql -h localhost -p 5432 -U postgres -d coaches_db -c \
  "ALTER DATABASE coaches_db SET default_transaction_read_only = on;"
```
...
SELECT * FROM pg_stat_activity WHERE datname = 'coaches_db';
...
PGPASSWORD="your-db-password" psql -h localhost -p 5432 -U postgres -d coaches_db -c \
  "ALTER DATABASE coaches_db SET default_transaction_read_only = off;"

# 2. Import recovered data
PGPASSWORD="your-db-password" psql -h localhost -p 5432 -U postgres -d coaches_db -f ./recovered_data.sql
...
```bash
sudo systemctl status postgresql
sudo systemctl status coaches-server
df -h
free -m
```
...
```bash
# Create backup
PGPASSWORD="password" pg_dump -h localhost -p 5432 -U postgres -d coaches_db -F c -f backup.sql.gz
...
```bash
# Create database
PGPASSWORD="password" createdb -h localhost -p 5432 -U postgres coaches_db

# Restore backup
PGPASSWORD="password" pg_restore -h localhost -p 5432 -U postgres -d coaches_db --clean --if-exists backup.sql.gz

# Verify restore
PGPASSWORD="password" psql -h localhost -p 5432 -U postgres -d coaches_db -c "\dt"
```
...
- Application Logs: `/var/log/coaches/`
